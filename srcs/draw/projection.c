...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/23 12:49:08 by mipinhei          #+#    #+#             */
/*   Updated: 2025/07/23 13:14:21 by mipinhei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			int *line_x, int *line_y)
{
	...
	*line_x = (int)((pt_x - pt_y) * cos(iso_angle)
			* data->view.zoom + data->view.x_offset);
	*line_y = (int)((pt_x + pt_y) * sin(iso_angle)
			* data->view.zoom - pt_z + data->view.y_offset);
}
...
void	ft_center_map(t_data *data)
{
	...
	while (++y < data->map->map_y)
	{
		...
		while (++x < data->map->map_x)
			center_util(data, &screen, &data->map->map_points[y][x]);
	}
	data->map->map_center_x = data->map->min_x
		+ (data->map->max_x - data->map->min_x) / 2;
	data->map->map_center_y = data->map->min_y
...
void	center_util(t_data *data, t_coordinate *screen, t_point *map_pts)
{
	if (data->projection == ISO)
		isometric_projection(data, map_pts,
			&screen->x, &screen->y);
	else
		ortographic_projection(data, map_pts, &screen->x, &screen->y);
